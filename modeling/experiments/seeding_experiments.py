...
class ContagionAnalyzer:
    ...
    def run_parameter_strategy_sweep(self, n_iterations: int = 10, shuffle: bool = True,
        ...
        """Run parameter sweep over network configurations AND seeding strategies."""
        print("\n" + "="*70)
    ...
    def _run_config(self, n_comms: float, pref_att: float) -> Optional[List[Dict]]:
        ...
        characteristic_groups = ["geslacht", "etngrp_geslacht_lft_oplniv", "geslacht_oplniv", "lft", "etngrp_oplniv"]
    ...
    def _run_config_with_strategies(self, n_comms: float, pref_att: float,
        ...
        """Run simulation for single parameter configuration with multiple seeding strategies."""
        folder = Path(self.net.folder_path(n_comms, pref_att))
        ...
        try:
            network_graphs = load_networks(str(folder), add_random=False)
            characteristic_groups = ["geslacht", "etngrp_geslacht_lft_oplniv", "geslacht_olpniv", "lft"]
            # networks = {key: network_graphs[key].graph for key in network_graphs}
            networks = {key: network_graphs[key].graph 
                        for key in network_graphs 
                        if key in (characteristic_groups)}
        ...
        # Calculate strategic seeding groups
    ...
    def _sweep_contested(self, networks: Dict) -> Tuple[Dict, Dict]:
        ...
        for name, G in networks.items():
            ...
            for i, tau in enumerate(self.sim.thresholds):
                ...
                finals[i] = {}
                finals[i]["mean"] = np.mean([ts[-1] for ts in ts_list])
                finals[i]["variance"] = np.var([ts[-1] for ts in ts_list])
...
def run_strategic_seeding(characteristics: List[str] = None, 
    ...
    if network_folder is None:
        network_folder = "Data/networks/werkschool/scale=0.1_comms=1.0_recip=1_trans=0_pa=0.0_bridge=0"
...
def run_parameter_strategy_sweep(n_iterations: int = 10,
    ...
    """Run and visualize combined parameter and strategy sweep analysis."""
    if characteristics is None:
        characteristics = ["geslacht", "lft", "etngrp", "oplniv"]
    if strategies is None:
        strategies = ['combined_score', 'internal_density', 'external_exposure_ratio', 
    ...
    out_path = Path(output_dir)
    out_path.mkdir(parents=True, exist_ok=True)
    results.to_csv(out_path / "param_strategy_results.csv", index=False)
    print(f"\nSaved results to {out_path / 'param_strategy_results.csv'}")
...
if __name__ == "__main__":
    ...
    print("="*70)
    sweep_results = run_parameter_sweep(n_iterations=100)
    print("\nSummary:")
    ...
    # Run combined parameter and strategy sweep
    # print("\n" + "="*70)
    # print("PARAMETER + STRATEGY SWEEP")
    # print("="*70)
    # param_strategy_results = run_parameter_strategy_sweep(n_iterations=20)
